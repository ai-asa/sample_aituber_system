...
class PlaySound:
    ...
    def _search_output_device_id(self, output_device_name, output_device_host_api=0) -> int:
        ...
        if output_device_id is None:
            print("指定のoutput_deviceが見つかりませんでした")
            exit()
...
def subprocess_play(queue):
    config = configparser.ConfigParser()
    config.read("settings.ini", encoding='utf-8')
    virtual_audio_cable = config.get('SYSTEM', 'virtual_audio_cable',fallback="CABLE Input")
    ps = PlaySound(output_device_name=virtual_audio_cable)
    while True:
        tuple_data = queue.get()
        data,rate = tuple_data
